...
var linkList = [ 'http://www.teatrobreton.org/rss_eventos.asp',
                 ...
                 'http://www.narodni-divadlo.cz/eventfeed',
                 'http://www.mdb.cz/rss'];
...
var newMeta = function(metaData) {
  if(MongoClient.prototype.db) {
    ...
    siteColl.insert(metaData, function(err, res) {
...
exports.clearMeta = function() {
  console.log('clearMeta operational!');
  MongoClient.connect(mongoUrl, function(err, db) {
    if(err) {
    ...
    siteColl.drop(function(err) {
      if(err) {
        console.log('The site collection could not be dropped, vole.');
      }
    });
  });
}
